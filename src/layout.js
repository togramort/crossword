function CrosswordPuzzle()
{
    ...
    let isWordComplete = function(word) {
        // Проверяем, что все буквы слова уже есть на сетке
        for (let i = 0; i < this.gridSize; i++) {
          for (let j = 0; j < this.gridSize; j++) {
            if (this.grid[i][j] === word.text[0]) {
              let isVertical = this.isVerticalWord(i, j);
              let index = 0;
              for (let k = 0; k < word.length; k++) {
                let row = isVertical ? i + k : i;
                let col = isVertical ? j : j + k;
                if (this.grid[row][col] !== word.text[index]) {
                  return false;
                }
                index++;
              }
